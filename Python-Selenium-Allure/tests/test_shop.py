...
def test_products_group(browser):
    ...
    for item in expected_menu:
        ...
        element.click()
        WebDriverWait(browser, timeout=10, poll_frequency=1).until(EC.url_to_be(f"{URL}{item[1]}"))

    result = [el.get_attribute('text') for el in elements]

    assert [e[0] for e in expected_menu] == result, 'Top menu does not matching to expected'
